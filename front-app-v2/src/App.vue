        ...
        <template v-else-if="currentPage === 'history'">
          ...
          <section class="panel">
            ...
            <div class="history-scroll-panel">
              <article v-for="item in historyStreams" :key="item.id" class="history-card">
                <div class="history-card-main">
                  ...
                  <div class="history-badges">
                    <span :class="['badge', historyDirectionOf(item) === 'rx' ? 'idle' : 'ok']"><i></i>{{ historyDirectionOf(item).toUpperCase() }}</span>
                    <span class="badge idle"><i></i>{{ historyDirectionOf(item) === 'rx' ? '收包' : (item.mode === 'pcap' ? 'PCAP' : '构造包') }}</span>
                    <span v-if="historyDirectionOf(item) !== 'rx'" class="badge idle"><i></i>{{ formatHistoryRates(item) }}</span>
                  </div>
                </div>

                <div class="history-config-compact">
                  <span v-for="chip in historyConfigChips(item)" :key="chip.label" :class="['history-chip', chip.kind ? `history-chip-${chip.kind}` : '']" :title="chip.value">
                    <em>{{ chip.label }}</em>
                    <strong>{{ chip.value }}</strong>
                  </span>
                </div>

                <div class="history-restore-grid">
                  <label class="field history-device-field">
                    <span>恢复设备</span>
                  ...
                  <label class="field history-core-field">
                    <span>核数</span>
                  ...
                  <label v-if="historyDirectionOf(item) !== 'rx'" class="field history-core-field">
                    <span>配置核</span>
                    <select v-model.number="item.restore_selected_worker_index" @change="clampHistoryWorkerIndex(item)">
                      <option v-for="(_, index) in item.restore_worker_rates_mbps" :key="index" :value="index">核 {{ index + 1 }}</option>
                    </select>
                  </label>
                  <label v-if="historyDirectionOf(item) !== 'rx'" class="field history-value-field">
                    <span>每核速率</span>
                    <input
                      v-model.number="item.restore_worker_rates_mbps[selectedHistoryWorkerIndex(item)]"
                      type="number"
                      min="100"
                      max="10000"
                      inputmode="numeric"
                      @change="clampHistoryRate(item, selectedHistoryWorkerIndex(item))"
                    >
                  </label>
                  <label v-if="historyDirectionOf(item) !== 'rx'" class="field history-value-field">
                    <span>突发区</span>
                    <input
                      v-model.number="item.restore_worker_burst_bytes[selectedHistoryWorkerIndex(item)]"
                      type="number"
                      min="2048"
                      max="16384"
                      step="1024"
                      inputmode="numeric"
                      @change="clampHistoryBurst(item, selectedHistoryWorkerIndex(item))"
                    >
                  </label>
                  <label v-else class="check-row history-dump-field"><input v-model="item.restore_pcap_dump_enabled" type="checkbox">pcap_dump</label>
              ...
              <div v-if="!historyStreams.length" class="history-empty">
                {{ historyEnabled ? '暂无历史 stream' : '数据库未启用或不可用' }}
              </div>
            </div>
          </section>
        </template>
...
function endpointPair(left, right) {
  return `${left || '-'} -> ${right || '-'}`
}

function compactIpLine(item, side) {
  const config = historyConfig(item)
  const mode = sequenceModeLabel(config[`${side}_ip_mode`])
  const step = config[`${side}_ip_mode`] === 'increment' ? ` +${formatNumber(config[`${side}_ip_step`])}` : ''
  return `${config[`${side}_ip_addr`]}/${config[`${side}_ip_mask`]} ${mode}${step}`
}

function compactPortLine(item, side) {
  const config = historyConfig(item)
  const start = config[`${side}_port_start`]
  const end = config[`${side}_port_end`]
  const range = start === end ? `${start}` : `${start}-${end}`
  const mode = sequenceModeLabel(config[`${side}_port_mode`])
  const step = config[`${side}_port_mode`] === 'increment' && start !== end ? ` +${formatNumber(config[`${side}_port_step`])}` : ''
  return `${range} ${mode}${step}`
}

function historyConfigChips(item) {
  const config = historyConfig(item)
  if (historyDirectionOf(item) === 'rx') {
    return [
      { label: 'RX 口', value: config.rx_port || '-', kind: 'medium' },
      { label: 'Dump', value: config.pcap_dump_enabled ? '开启' : '关闭', kind: 'small' },
      { label: '目录', value: shortPath(runtime.value.settings?.rx_pcap_dump_root || runtime.value.rx_pcap_dump_root || '-'), kind: 'wide' }
    ]
  }
  if (item.mode === 'pcap') {
    return [
      { label: 'PCAP', value: shortPath(config.pcap_path || '-'), kind: 'wide' },
      { label: '循环', value: config.loop ? '开启' : '关闭', kind: 'small' },
      { label: '协议', value: `${config.l3 || 'IPv4'} / ${config.l4 || 'UDP'}`, kind: 'small' }
    ]
  }
  return [
    { label: 'MAC', value: endpointPair(config.src_mac, config.dst_mac), kind: 'wide' },
    { label: 'IP', value: endpointPair(compactIpLine(item, 'src'), compactIpLine(item, 'dst')), kind: 'wide' },
    { label: '端口', value: endpointPair(compactPortLine(item, 'src'), compactPortLine(item, 'dst')), kind: 'medium' },
    { label: '负载', value: `${formatNumber(config.payload_len)} B`, kind: 'small' }
  ]
}

function selectedHistoryWorkerIndex(item) {
  const maxIndex = Math.max(0, (item.restore_worker_rates_mbps?.length || 1) - 1)
  return clampInt(item.restore_selected_worker_index ?? 0, 0, maxIndex, 0)
}

function clampHistoryWorkerIndex(item) {
  item.restore_selected_worker_index = selectedHistoryWorkerIndex(item)
}
...
async function refreshHistory() {
  ...
  historyStreams.value = (result.streams || []).map((item) => ({
    ...
    restore_selected_worker_index: 0
  }))
}
